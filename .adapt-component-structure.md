...
│   ├── componentName.jsx            # JSX/React template (Adapt v5+)
│   └── ...other-files.js            # Additional helpers
│
├── templates/                       # Template files (LEGACY ONLY)
│   └── componentName.hbs            # Handlebars template (Adapt v4 and below)
│
...
### 2. Template Format & Location
- ✅ **CORRECT**: JSX/React templates (`.jsx`) for modern Adapt (v5+)
  - **CRITICAL**: JSX templates must be in `js/` folder, NOT `templates/` folder
  - Import with: `import template from './componentName.jsx';`
- ⚠️ **LEGACY**: Handlebars templates (`.hbs`) in `templates/` folder for older versions
...
- **Template reference for JSX** (Adapt v5+):
```javascript
import template from './componentName.jsx';
ComponentNameView.template = template;
```
- **Template reference for Handlebars** (Legacy):
```javascript
ComponentName.template = 'componentName'; // NO extension
```
...
2. ❌ **JSX templates in `templates/` folder** - Must be in `js/` folder!
3. ❌ Using string reference for JSX templates (use `import` instead)
4. ❌ Using `properties-schema/` instead of `schema/`
5. ❌ Not creating GitHub releases/tags
6. ❌ Including too much documentation (keep only README.md)
7. ❌ Wrong main entry file path in bower.json/package.json
8. ❌ Template extension in Handlebars template reference (should be no extension)
9. ❌ Not following conventional commit format
